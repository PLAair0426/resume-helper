 ...
 * code blocks (```json ... ```) or contain trailing text after the JSON.
 */
      ...
      POST: async ({ request }) => {
        try {
          ...
          const response = await fetch(modelConfig.url(apiEndpoint), {
            ...
            **返回纯 JSON**，不要用 markdown 代码块包裹，不要在 JSON 前后添加任何文字说明。格式：
            {
              "errors": [
                {
            ...
            再次强调：**只找错别字和标点错误，不要做任何润色！返回纯 JSON！**`
                },
                {
                  role: "user",
                  content
          ...
          if (!response.ok) {
            ...
            try {
              ...
              if (errorText.trim()) errorMessage = errorText.slice(0, 300);
            }
          ...
          // Normalize: extract JSON from potentially code-block-wrapped content
          const rawContent = data.choices?.[0]?.message?.content;
          if (typeof rawContent === "string") {
            data.choices[0].message.content = extractJsonFromContent(rawContent);
          }
