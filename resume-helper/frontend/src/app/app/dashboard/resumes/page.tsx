...
import { ResumeImportPanel } from "@/components/agent/ResumeImportPanel";
...
const ResumeWorkbench = () => {
  ...
  const [importTemplateId, setImportTemplateId] = React.useState<string>(
  ...
  const handleOpenImportModal = () => {
    if (!DEFAULT_TEMPLATES.length) {
      toast.error(t("dashboard.resumes.importError"));
      return;
    }

    setImportTemplateId(DEFAULT_TEMPLATES[0].id);
    setIsImportModalOpen(true);
  };

  const handleImportProfile = (profile: ResumeProfile) => {
    try {
      const targetTemplateId = importTemplateId || DEFAULT_TEMPLATES[0]?.id || null;
      const newId = createResume(targetTemplateId, true);
      const createdResume = useResumeStore.getState().resumes[newId];

      if (!createdResume) {
        throw new Error("New resume is missing after creation");
      }

      const merged = fromResumeProfile(profile, createdResume);
      const importedName = profile?.basics?.name?.trim();
      const importedTitle = importedName || createdResume.title;
            ...
            <div className="px-6 pb-6 overflow-y-auto space-y-4">
              <div className="space-y-2">
                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">
                  {t("dashboard.resumes.importDialog.templateLabel")}
                </p>
                <Select
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue
                      placeholder={t("dashboard.resumes.importDialog.templatePlaceholder")}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {DEFAULT_TEMPLATES.map((template) => (
                      <SelectItem key={template.id} value={template.id}>
                        {t(`dashboard.templates.${toTemplateNameKey(template.id)}.name`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {t("dashboard.resumes.importDialog.supportHint")}
              </p>

              <ResumeImportPanel
                templateId={importTemplateId}
                onImport={(profile) => handleImportProfile(profile as ResumeProfile)}
                importButtonText={t("dashboard.resumes.importDialog.importAsNew")}
              />
            </div>
          </DialogContent>
        </Dialog>
