...
import { AI_MODEL_CONFIGS, getEffectiveEndpoint } from "@/config/ai";
...
export const useGrammarStore = create<GrammarStore>((set, get) => ({
  ...
  checkGrammar: async (text: string) => {
    ...
    const apiEndpoint =
      creds.endpointMode === "custom"
        ? getEffectiveEndpoint(selectedModel, creds)
        : undefined;
    ...
    try {
      const response = await fetch("/api/grammar", {
        ...
        body: JSON.stringify({
          content: text,
          apiKey: creds.apiKey,
          model: config.requiresModelId ? creds.modelId : config.defaultModel,
      ...
      if (data.error) {
        const errMsg =
          typeof data.error === "string"
            ? data.error
            : data.error?.message || "Grammar check failed";
        toast.error(errMsg);
